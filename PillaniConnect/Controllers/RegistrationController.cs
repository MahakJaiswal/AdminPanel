...
*/        public RegistrationController(/*IHubContext<UserHub> hubContext,*/ IWebHostEnvironment environment, IConfiguration configuration)
          {
...
*/
        ...
        {
            Cls_Registration obj = new Cls_Registration();
            obj._List = new List<Cls_Registration>();
            DataTable dt = obj.Get_Registered_Members();
            obj._List = BindUser(dt);
            return View(obj);
        }
        public List<Cls_Registration> BindUser(DataTable dt)
        {
            List<Cls_Registration> lst = new List<Cls_Registration>();
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    var url = HttpContext.Request.Scheme + "://" + HttpContext.Request.Host + "/api/GetEmployeeImageBlob/" + dr["sysreguuid"].ToString();

                    lst.Add(new Cls_Registration()
                    {
                        //LineId = Convert.ToInt32(dr["LineID"].ToString()),
                        ReegUuid = dr["sysreguuid"].ToString(),
                        lineid = Convert.ToInt32(dr["lineid"].ToString()),
                        TotalUsers = Convert.ToInt32(dr["total_user_count"].ToString()),
        ...
        [HttpPost]
        public async Task<IActionResult> RegisteredMembers(Cls_Registration _obj, Microsoft.AspNetCore.Http.IFormFile postedFile, IFormCollection form, string save, string Upload, string Update, string status, string Confirm)
        {

            List<Cls_Registration> _uploadList = new List<Cls_Registration>();
            if (ModelState.IsValid)
            {
                ...
                if (!string.IsNullOrEmpty(save))
                {
                    ...
                    if (postedFile != null)
                    {
                        string path = Path.Combine(Environment.ContentRootPath, "postedFile");
                   ...
                   /* await _hubContext.Clients.All.SendAsync("NewUserAdded", _obj.lineid);
                    return RedirectToAction("RegisteredMembers");*/


                }

                DataTable dt = _obj.Get_Registered_Members();
                _obj._List = BindUser(dt);
        ...
        {
            Cls_Registration obj = new Cls_Registration();
            DataTable dt = obj.Get_designation_DD();
            DataTable dt1 = obj.Get_Industry_DD();
            DataTable dt2 = obj.Get_Country_DD();
            DataTable dt3 = obj.Get_State_DD();
            DataTable dt4 = obj.Get_City_DD();


            obj.DesignationDDL = BindDesignation(dt);
            obj.IndustryDDL = BindIndustry(dt1);
            obj.CountryDDL = BindCountry(dt2);
            obj.StateDDL = BindState(dt3);
            obj.CityDDL = BindCity(dt4);



            return PartialView(obj);
        }
        public List<SelectListItem> BindDesignation(DataTable dt)
        {
        ...
        public List<SelectListItem> BindIndustry(DataTable dt)
        {
            List<SelectListItem> NameList = new List<SelectListItem>();
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        public List<SelectListItem> BindState(DataTable dt)
        {
            List<SelectListItem> NameList = new List<SelectListItem>();
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        public List<SelectListItem> BindCity(DataTable dt)
        {
            List<SelectListItem> NameList = new List<SelectListItem>();
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        public JsonResult GetStateByCountry(string Country)
        {
            ...
            _lst = BindState(new Cls_Registration().Get_State_By_Country(Country));
            return Json(_lst);
        }
        [HttpGet]
        public JsonResult GetCityByState(string State)
        {

            List<SelectListItem> _lst = new List<SelectListItem>();
            _lst = BindCity(new Cls_Registration().Get_City_By_State(State));
            return Json(_lst);
        }

        public IActionResult ProfileMaster()
        {
            Cls_Registration obj = new Cls_Registration();
            obj._DetailsList = new List<Cls_Registration>();
            DataTable dt = obj.Get_Registered_Members_Details((HttpContext.Session.GetString("SysUserUUid")));
            obj._DetailsList = BindUserDetails(dt);
            return View(obj);
        }

        public List<Cls_Registration> BindUserDetails(DataTable dt)
        {
            List<Cls_Registration> lst = new List<Cls_Registration>();
            if (dt != null && dt.Rows.Count > 0)
            {
        ...
        {
            Cls_Registration _obj = new Cls_Registration();
            DataTable dt = new DataTable();

            dt = _obj.Get_Registered_Members_Details(UserId);
            if (dt != null && dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {

                    _obj.lineid = Convert.ToInt32(dr["lineid"].ToString());
                    _obj.Name = dr["Name"].ToString();
                    _obj.Emailid = dr["emailid"].ToString();
                    _obj.Contact1 = dr["contact1"].ToString();
                    _obj.Contact2 = dr["contact2"].ToString();
                    _obj.Designation = dr["designationname"].ToString();
                    _obj.Industry = dr["industryname"].ToString();
                    _obj.Address = dr["address"].ToString();
                    _obj.Company = dr["company"].ToString();
                    _obj.City = dr["city"].ToString();
                    _obj.Pincode = dr["pincode"].ToString();
                    _obj.description = dr["description"].ToString();
                    //_obj.photo = url;
                    _obj.postedby = dr["postedby"].ToString();
                }

            }
            return PartialView("EditUserProfile", _obj);
        }
        ...
        public IActionResult UserProfile(string UserId)
        {
            Cls_Registration obj = new Cls_Registration();
            obj._DetailsList = new List<Cls_Registration>();
            DataTable dt = obj.Get_Registered_Members_Details(UserId);
            obj._DetailsList = BindUserDetails(dt);
            return PartialView("UserProfile", obj);
        }
